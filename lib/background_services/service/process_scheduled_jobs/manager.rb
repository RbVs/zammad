      ...
      class Manager
        ...
        def wrapup
          if job.present?
            ...
            Rails.logger.warn ' ...Job got deleted while running'
          end
        end

        def invalid_thread_log(thread, status)
          message = if thread.respond_to?(:status)
                      "Invalid thread stored for job '#{job.name}' (#{job.method}): #{thread.inspect}. Deleting and resting job."
                    else
                      how = if status.nil?
                              'via an exception'
                            elsif status == false
